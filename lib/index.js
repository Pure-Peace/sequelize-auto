  ...
  function generateText(err) {
    ...
    async.each(_.keys(self.tables), function (table, _callback) {
      ...
      if (!self.options.typescript) {
        ...
        text[table] = tsHelper.model.getModelFileStart(self.options.indentation, spaces, tableName, self.options.delegate || 'model');
      }
      ...
      _.each(fields, function (field, i) {
        ...
        _.each(fieldAttr, function (attr, x) {
          ...
          if (attr === "foreignKey") {
            ...
            const column = self.tables[table][field];
            if (column && column.foreignKey && column.foreignKey.EXTRA === 'auto_increment') {
              text[table] += spaces + spaces + spaces + "autoIncrement: true,\n";
              autoIncrement = true;
            }
        ...
        if (self.options.typescript) tsTableDef += tsHelper.def.getMemberDefinition(spaces, fieldName, tsVal, tsAllowNull, autoIncrement);
      });
      ...
      if (fields.includes('createdAt') || fields.includes('updatedAt')) {
        if (fields.includes('createdAt')) {
          text[table] += spaces + spaces + "createdAt: 'createdAt',\n";
        }
        if (fields.includes('updatedAt')) {
          text[table] += spaces + spaces + "updatedAt: 'updatedAt',\n";
        }
        text[table] += spaces + spaces + "timestamps: true,\n";
      }
      if (fields.includes('deletedAt')) {
        text[table] += spaces + spaces + "deletedAt: 'deletedAt',\n";
        text[table] += spaces + spaces + "paranoid: true,\n";
      }
      text[table] += spaces + spaces + "tableName: '" + _.snakeCase(table) + "',\n";
      ...
      var eggDefs = tsHelper.def.getEggDefines(self.options.delegate || 'model', tableNames);
      typescriptFiles[0] += eggDefs;
...
AutoSequelize.prototype.write = function (attributes, typescriptFiles, callback) {
  ...
  if (self.options.typescript) {
    if (typescriptFiles !== null && typescriptFiles.length > 1) {
      fs.writeFileSync(path.join(self.options.directory, 'index.d.ts'), typescriptFiles[0], 'utf8');
